...
<main class="flex-1 overflow-y-auto bg-gray-50 p-6 ml-16 md:ml-64 mt-[56px]">
    ...
    <div id="appointmentsSection" class="hidden">
        ...
        $pending_stmt = $conn->prepare("SELECT a.date, a.time, a.reason, a.status, a.email, ip.name, ds.doctor_name 
                                        FROM appointments a 
                                        JOIN imported_patients ip ON a.student_id = ip.id 
        ...
        $done_total_count_stmt = $conn->query("SELECT COUNT(*) FROM appointments a JOIN imported_patients ip ON a.student_id = ip.id WHERE a.status IN ('approved', 'confirmed')");

        $done_total_records = $done_total_count_stmt->fetch_row()[0];

        $done_total_pages = ceil($done_total_records / $done_records_per_page);

        $done_stmt = $conn->prepare("SELECT a.date, a.time, a.reason, a.status, a.email, ip.name FROM appointments a JOIN imported_patients ip ON a.student_id = ip.id WHERE a.status IN ('approved', 'confirmed') ORDER BY a.date DESC, a.time DESC LIMIT ? OFFSET ?");
        ...
        $declined_total_count_stmt = $conn->query("SELECT COUNT(*) FROM appointments a JOIN imported_patients ip ON a.student_id = ip.id WHERE a.status = 'declined'");
        ...
        $declined_stmt = $conn->prepare("SELECT a.date, a.time, a.reason, a.status, a.email, ip.name FROM appointments a JOIN imported_patients ip ON a.student_id = ip.id WHERE a.status = 'declined' ORDER BY a.date DESC, a.time DESC LIMIT ? OFFSET ?");
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.edit-schedule-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                const scheduleDate = this.dataset.date;
                const scheduleTime = this.dataset.time;
